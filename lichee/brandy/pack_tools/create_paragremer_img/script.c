...
int script_parser_fetch_partition(void)
{
	...
	/* 检查脚本buffer是否存在 */
	/* 先找partition_start，然后找其后面的partition */
	if (!partition_start) {
		for (i = 0; i < script_main_key_count; i++) {
			main_key =
			    (script_main_key_t *)(script_mod_buf +
						  (sizeof(script_head_t)) +
						  i * sizeof(
							  script_main_key_t));
			/* 如果主键不匹配，寻找下一个主键 */
			if (strcmp(main_key->main_name, "partition_start")) {
				continue;
			} else {
				partition_start = i;
	...
	partition_next++;
	main_key =
	    (script_main_key_t *)(script_mod_buf + (sizeof(script_head_t)) +
				  partition_next * sizeof(script_main_key_t));
...
int script_parser_fetch_mainkey_sub(char *sub_name, int index, int *value)
{
	...
	for (j = 0; j < main_key->lenth; j++) {
		...
		pattern =
		    (sub_key->pattern >> 16) & 0xffff; /* 获取数据的类型 */
		word_count =
		    (sub_key->pattern >> 0) & 0xffff; /* 获取所占用的word个数 */
		/* 取出数据 */
		switch (pattern) {
		case DATA_TYPE_SINGLE_WORD: /* 单word数据类型 */
			value[0] =
			    *(int *)(script_mod_buf + (sub_key->offset << 2));
			break;
