...
void auto_set_timing_para(__dram_para_t *para)
{
	...
	case 3://DDR3
		...
		t_rdata_en  =5;
		wr_latency  =3;
		para->dram_mr0 	= 0x1E14;//CL=13,WR=14
		para->dram_mr2     = 0x20; //CWL=9
		para->dram_mr3     = 0;
#else
		tcl		= 6;	//CL   12
		tcwl	= 4;	//CWL  8
...
unsigned int mctl_init(void)
{
	...
	//set the parameter
#if defined DRAMC_MASTER_TEST && (defined DRAM_TYPE_DDR3)// || defined DRAM_TYPE_LPDDR3)
	dram_para.dram_clk			= 936;
#else
...
unsigned int mctl_channel_init(unsigned int ch_index,__dram_para_t *para)
{
	...
	case 2:
		...
		break;
	case 3:
		mctl_write_w(0x4319E5,MC_WORK_MODE);//map0;default 0x4308E5--16bit
		mctl_write_w(mctl_read_w(_DRAM_CLK_REG)|(0x1U<<31),_DRAM_CLK_REG);
		mctl_write_w(0x00310048,RFSHTMG);
		mctl_write_w(0x1c70,DRAM_MR0);
		mctl_write_w(0x40,DRAM_MR1);
		mctl_write_w(0x18,DRAM_MR2);
		mctl_write_w(0,DRAM_MR3);
		mctl_write_w(0x42C21590,PTR0);
		mctl_write_w(0x00083DEF,PTR2);
		mctl_write_w(0x0A431CE1,PTR3);
		mctl_write_w(0x19913EC1,PTR4);
		mctl_write_w(0x81003087,DTCR);
		mctl_write_w(0x4680C620,PGCR1);
		mctl_write_w(0x02040102,PITMG0);
		mctl_write_w(0x0C0B1808,DRAMTMG0);
		mctl_write_w(0x0003040B,DRAMTMG1);
		mctl_write_w(0x0406050A,DRAMTMG2);
		mctl_write_w(0x0000400C,DRAMTMG3);
		mctl_write_w(0x04020404,DRAMTMG4);
