...
static void sunxi_snd_txctrl(struct snd_pcm_substream *substream, int on, struct snd_soc_dai *dai)
{
	...
	if (on) {
		...
		/* global enable */
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_CTL, 0x1, GEN, 1);
		gen_use++;
	} else {
		...
		/* global disable */
		if (--gen_use  == 0)
			codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_CTL, 0x1, GEN, 0);
	}
}
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_RXCHSEL,
