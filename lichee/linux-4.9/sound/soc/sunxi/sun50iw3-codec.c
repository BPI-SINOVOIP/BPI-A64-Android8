...
static void dump_ccmu_audio(void)
{
	...
	pr_debug("[%s] --> AUDIO_CODEC_1X_CLK_REG=0x%x\n",
			__func__, readl(ioremap(0x03001a50, 1)));
	/* AUDIO_CODEC_4X_CLK_REG */
	pr_debug("[%s] -->  AUDIO_CODEC_4X_CLK_REG=0x%x\n",
			__func__, readl(ioremap(0x03001a54, 1)));
	/* AUDIO_CODEC_BGR_REG */
	pr_debug("[%s] --> AUDIO_CODEC_BGR_REG=0x%x\n",
			__func__, readl(ioremap(0x03001a5c, 1)));
}
#endif
...
static int codec_suspend(struct snd_soc_codec *codec)
{
	...
	/* we can reset codec after save these regs!*/
							...
							struct device_attribute *attr, char *buf)
{
	...
	while (reg_labels[i].name != NULL) {
		...
		if (reg_group == 1) {
			...
			count += sprintf(buf + count, "%s 0x%lx: 0x%x, Save:0x%x\n",
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sunxi_internal_codec->vol_supply.vdd_sys)) {
		pr_err("[%s]:get audio vdd-sys failed\n", __func__);
		ret = -EFAULT;
		goto err1;
	} else {
		ret = regulator_enable(sunxi_internal_codec->vol_supply.vdd_sys);
		if (ret) {
			pr_err("[%s]: vdd-sys:enable() failed!\n", __func__);
			goto err1;
		}
	}
#if 0
	...
	if (IS_ERR(sunxi_internal_codec->pllclk)
		...
		if (clk_set_parent(sunxi_internal_codec->moduleclk,
			...
			pr_err("try to set parent of sunxi_spdif->moduleclk to sunxi_spdif->pllclk failed!\n");
