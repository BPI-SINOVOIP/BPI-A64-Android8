...
#define PARTITION_FROM_CMDLINE false
#if PARTITION_FROM_CMDLINE
    std::vector<std::string> cmd_links;
    ...
    if (getline(cmdline_fin, cmdline)) {
        ...
        for (i = 0; i < len; i = j + 1) {
            ...
            if (!strcmp(uevent.device_name.c_str(), blk.c_str()))
                cmd_links.emplace_back("/dev/block/by-name/" + name);
        }
    }
    return cmd_links;
#endif
	...
	std::string link_path = "/dev/block/";
