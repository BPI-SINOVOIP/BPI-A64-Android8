...
void HealthdDraw::draw_battery(const animation* anim) {
 ...
 void HealthdDraw::draw_capacity(const animation* anim)
{
    char cap_str[STR_LEN];
    int x, y;
    int str_len_px;
    int h;
    android::animation::frame *frame = &anim->frames[anim->cur_frame];

    snprintf(cap_str, (STR_LEN - 1), "%d%%", anim->cur_level);
    str_len_px = gr_measure(gr_sys_font(), cap_str);
    x = (gr_fb_width() - str_len_px) / 2;
    h = gr_get_height(frame->surface);
    y = (gr_fb_height() + h + char_height_) / 2 ;
