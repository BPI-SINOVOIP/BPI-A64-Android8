...
status_t PublicVolume::doMount() {
    ...
    if (mFsType != "vfat" && mFsType != "ntfs" && mFsType != "exfat") {
    ...
    } else if (!exfat::Check(mDevPath)) {
        LOG(INFO) << getId() << " check exfat filesystem OK";
    } else {
    ...
    } else if (!exfat::Mount(mDevPath, mRawPath, false, false, false,
            AID_MEDIA_RW, AID_MEDIA_RW, 0007, false)) {
        LOG(INFO) << getId() << " mount exfat filesystem OK";
    } else {
