...
final class WiredAccessoryManager implements WiredAccessoryCallbacks {
    ...
    private static boolean is_hdmi = false;
    ...
    private void updateLocked(String newName, int newState) {
        ...
        int hdmi = headsetState & BIT_HDMI_AUDIO;
        ...
        // - c: 0 usb headset to 1 usb headset
        // - d: 1 usb headset to 0 usb headset
        if (hdmi == BIT_HDMI_AUDIO) {
            Log.e(TAG, "Invalid combination, unsetting hdmi flag");
    ...
    class WiredAccessoryObserver extends UEventObserver {
            ...
            synchronized (mLock) {
                ...
                for (int i = 0; i < mUEventInfo.size(); ++i) {
                    ...
                    try {
                        ...
                        file.close();
                        String s = new String(buffer, 0, len-1);
						// distinguish with HDMI plug
                        // substring to extract integer from "HDMI=1"
                        if (is_hdmi = s.contains("HDMI"))
                            s = s.substring(5);
                        Slog.d(TAG, "WiredAccessoryObserver init s = " + s);

                        //curState = Integer.parseInt((new String(buffer, 0, len)).trim());
                        curState = Integer.parseInt(s.trim());
        ...
        public void onUEvent(UEventObserver.UEvent event) {
            ...
            try {
                ...
                String name;
                int state;
                if (devType.compareTo(NAME_HDMI) == 0 || devType.compareTo(NAME_HDMI_AUDIO) == 0) {
                    devPath = event.get("DEVPATH");
                    name = event.get("NAME");
                    String devState = event.get("STATE");//ex. "HDMI=1"
                    devState = devState.substring(5); //ex. "1"
                    state = Integer.parseInt(devState);
                } else {
                    devPath = event.get("DEVPATH");
        ...
        private final class UEventInfo {
            ...
            public String getDevPath() {
                if (!is_hdmi)
                    ...
                    return String.format(Locale.US, "/devices/platform/soc/%s/extcon/%s", mDevName, mDevName);
            }
