...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        Log.d(TAG, "hungUp=" + hungUp + "mScreenshotChordVolumeDownKeyTriggered=" + mScreenshotChordVolumeDownKeyTriggered
		+ " mA11yShortcutChordVolumeUpKeyTriggered="+ mA11yShortcutChordVolumeUpKeyTriggered
		+ " gesturedServiceIntercepted" + gesturedServiceIntercepted);
	if (!mPowerKeyHandled) {
            if (interactive) {
    ...
    void initializeHdmiState() {
        ...
        // modified by yangwenxiang@allwinnertech.com
        //if (new File("/sys/devices/virtual/switch/hdmi/state").exists()) {
        if (new File("/sys/class/switch/hdmi/state").exists()) {
            //mHDMIObserver.startObserving("DEVPATH=/devices/virtual/switch/hdmi");
            // use new devpath of hdmi uevent
            mHDMIObserver.startObserving("DEVPATH=/devices/platform/soc/hdmi");
            ...
            try {
                ...
                if (n > 1) {
                    String s = new String(buf, 0, n-1);//ex. "HDMI=1"
                    // substring to extract integer from "HDMI=1"
                    s = s.substring(5);//ex. "1"
                    plugged = 0 != Integer.parseInt(s);
                }
