...
namespace {
#include <stdio.h>
 char kSupplicantInitProperty[PROPERTY_VALUE_MAX] = {'\0'};
...
char  kSupplicantServiceName[PROPERTY_VALUE_MAX] = {'\0'};
char  wifi_hardware_info[PROPERTY_VALUE_MAX] = {'\0'};
...
bool SupplicantManager::StartSupplicant() {
  ...
  char *vendor_name;
  char *module_name;
  if (property_get("wlan.hardware.info", wifi_hardware_info, NULL) > 0) {
    vendor_name = strtok_r(wifi_hardware_info, ":",&module_name);
    if (vendor_name == NULL) {
      LOG(ERROR) << "unknown wifi module,return";
      return false;
    }
    snprintf(kSupplicantServiceName,sizeof(kSupplicantServiceName),"wpa_supplicant_%s",vendor_name);
  } else {
    LOG(ERROR) << "wlan.hardware.info is null,return";
    return false;
  }
  snprintf(kSupplicantInitProperty,sizeof(kSupplicantInitProperty),"init.svc.wpa_supplicant_%s",vendor_name);
