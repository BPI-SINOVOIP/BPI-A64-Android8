...
static create_aw_omx_component omx_core_load_cmp_library(int idx)
{
    ...
    {
        logv("Dynamically Loading the library : %s\n",core[idx].so_lib_name);
    ...
    if(core[idx].so_lib_handle)
    {
        if(core[idx].fn_ptr == NULL)
        {
            ...
            if(core[idx].fn_ptr == NULL)
            {
                logv("Error: Library %s incompatible as QCOM OMX component loader - %s\n",
...
static int get_cmp_index(char *cmp_name)
{
    ...
    logv("before get_cmp_index **********%d\n", rc);

    pthread_mutex_lock(&g_mutex_core_info);

    for(i=0; i< (int)NUM_OF_CORE; i++)
    {
        logv("get_cmp_index: cmp_name = %s , core[i].name = %s ,count = %d \n",
...
void* get_cmp_handle(char *cmp_name)
{
    ...
    logv("get_cmp_handle returning NULL \n");
                                                   ...
                                                   OMX_IN OMX_CALLBACKTYPE* callBacks)
{
    ...
    logv("OMXCORE API :  Get Handle %x %s %x\n",
    ...
    if(handle)
    {
        ...
        if(cmp_index >= 0)
        {
            ...
            // logv("core[cmp_index].fn_ptr: %x", core[cmp_index].fn_ptr);
            ...
            if(fn_ptr)
            {
                ...
                if(pThis)
                {
                    ...
                    logv("Component %x Successfully created\n",(unsigned)*handle);
                }
                else
                {
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_FreeHandle(OMX_IN OMX_HANDLETYPE hComp)
{
    ...
    logv("OMXCORE API :  Free Handle %x\n",(unsigned) hComp);
    ...
    if((i= is_cmp_handle_exists(hComp)) >=0)
    {
        ...
        {
            logv(" OMX_FreeHandle failed on %x\n",(unsigned) hComp);
            return eRet;
        }
    }
    else
    {
        logv("OMXCORE Warning: Free Handle called with no active instances\n");
    }
