...
typedef enum OMX_RENDER_COMMANDTYPE
{
    ...
    RENDER_THREAD_CMD_CONTINUE = 2,
    RENDER_THREAD_CMD_PAUSE  = 3,
    RENDER_THREAD_CMD_EXIT   = 4,
...
static void omxFreeOutputBuffer(omx_vdec* pSelf)
{
    int i, ret;
    for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
    {
        ...
        if(pSelf->mOutputBufferInfo[i].handle_ion != ION_NULL_VALUE)
        {
            ...
            ret = ion_free(pSelf->mIonFd, pSelf->mOutputBufferInfo[i].handle_ion);
            if(ret != 0)
                loge("ion_free( %d ) failed", pSelf->mIonFd);
            pSelf->mOutputBufferInfo[i].handle_ion = ION_NULL_VALUE;
        }
...
omx_vdec::omx_vdec()
{
    ...
    mqRenderThread = CdcMessageQueueCreate(64, "omx_vdec_new_RenderThread");
...
OMX_ERRORTYPE omx_vdec::component_init(OMX_STRING pName)
{
    ...
    */
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.hevc.secure",
             OMX_MAX_STRINGNAME_SIZE))
    {
    ...
    m_sInPortDefType.format.video.nFrameWidth          = WIDTH_DEFAULT;
    m_sInPortDefType.format.video.nFrameHeight          = HEIGHT_DEFAULT;
    ...
    m_sOutPortDefType.format.video.nFrameWidth          = WIDTH_DEFAULT;
    m_sOutPortDefType.format.video.nFrameHeight      = HEIGHT_DEFAULT;
    ...
    mOutBufferCntActual                               = MAX_NUM_OUT_BUFFERS;
                                       ...
                                       OMX_INOUT OMX_PTR     pParamData)
{
    ...
    logv("(f:%s, l:%d) eParamIndex = 0x%x", __FUNCTION__, __LINE__, eParamIndex);
    if(m_state == OMX_StateInvalid)
    {
    ...
    switch(eParamIndex)
    {
        ...
        case OMX_IndexParamPortDefinition:
        {
            ...
            {
                #ifdef __ANDROID__
                //* The configured color format should be consistent with ACodec where
                //* H265 AFBC is dealed with.
                if((m_sOutPortDefType.format.video.eColorFormat == OMX_COLOR_FormatYUV420Planar
                     || nActualColorFormat == (OMX_U32)PIXEL_FORMAT_YV12)
                     && m_useAndroidBuffer == OMX_TRUE)
                {
                    m_sOutPortDefType.format.video.eColorFormat =
                        (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YV12;
                }
                else if((m_sOutPortDefType.format.video.eColorFormat ==
                        OMX_COLOR_FormatYVU420SemiPlanar ||
                        nActualColorFormat == (OMX_U32)PIXEL_FORMAT_NV21)
                        && m_useAndroidBuffer == OMX_TRUE)
                {
                    m_sOutPortDefType.format.video.eColorFormat =
                        (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YCrCb_420_SP;
                }

                #endif
                memcpy(pParamData, &m_sOutPortDefType, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
                logv("(omx_vdec, f:%s, l:%d) OMX_IndexParamPortDefinition, width = %d, height = %d,\
        ...
        case OMX_IndexParamVideoPortFormat:
        {
            logv(" COMPONENT_GET_PARAMETER: OMX_IndexParamVideoPortFormat");
                  ...
                  == m_sInPortFormatType.nPortIndex)
             {
                ...
                {
                    loge("erro: pParamData->nIndex > m_sInPortFormatType.nIndex");
                     ...
                     == m_sOutPortFormatType.nPortIndex)
            {
                ...
                {
                    logw("warning: pParamData->nIndex > m_sOutPortFormatType.nIndex");
                    //eError = OMX_ErrorNoMore;
                ...
                {
                #ifdef __ANDROID__
                if((m_sOutPortFormatType.eColorFormat == OMX_COLOR_FormatYUV420Planar
                    || nActualColorFormat == (OMX_U32)PIXEL_FORMAT_YV12)
                    && m_useAndroidBuffer == OMX_TRUE)
                    {
                    m_sOutPortFormatType.eColorFormat
                        = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YV12;
                    }
                    else if((m_sOutPortDefType.format.video.eColorFormat ==
                        OMX_COLOR_FormatYVU420SemiPlanar ||
        ...
        case OMX_IndexParamPortDefinition:
        {
                 ...
                 == m_sInPortDefType.nPortIndex)
            {
                ...
                m_streamInfo.nHeight = m_sInPortDefType.format.video.nFrameHeight;
                     ...
                     == m_sOutPortDefType.nPortIndex)
            {
                OMX_U32 nSetBufferCntActual = ((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nBufferCountActual;
                logd("set_output_OMX_IndexParamPortDefinition,nSetBufferCntActual = %d, mOutBufferCntActual = %d",
                    nSetBufferCntActual, mOutBufferCntActual);
                if(nSetBufferCntActual != mOutBufferCntActual)
                {
                    int nBufCnt;
                    int nIndex;
                    ...
                    {
                        mExtraOutBufferNum = nSetBufferCntActual - mOutBufferCntActual;
                ...
                mOutBufferCntActual = nSetBufferCntActual;
        ...
        case OMX_IndexParamVideoPortFormat:
        {
                     ...
                     == m_sOutPortFormatType.nPortIndex)
            {
                if (((OMX_VIDEO_PARAM_PORTFORMATTYPE*)(pParamData))->nIndex
                    > m_sOutPortFormatType.nIndex)
                {
                    loge("erro: pParamData->nIndex > m_sOutPortFormatType.nIndex");
                }
                else
                {
                                    ...
                                    OMX_INOUT OMX_PTR pConfigData)
{
    ...
    logv("(f:%s, l:%d) index = %d", __FUNCTION__, __LINE__, eConfigIndex);
    if (m_state == OMX_StateInvalid)
    {
                                      ...
                                      OMX_IN    OMX_U8*                 pBuffer)
{
    ...
    else
    {
        ...
        if(m_storeOutputMetaDataFlag==OMX_TRUE)
        {
            ...
            {
                logw("warning: the nSizeBytes is not match: %lu, %d",
                      nSizeBytes,(int)sizeof(VideoDecoderOutputMetaData));
                //return OMX_ErrorBadParameter;
            }
        }
        else
        ...
        logv("use_buffer, m_sOutPortDefType.nPortIndex=[%d]", (int)m_sOutPortDefType.nPortIndex);
        pthread_mutex_lock(&m_outBufMutex);
        ...
        *ppBufferHdr = &m_sOutBufList.pBufArr[nIndex];
        ...
        {
            pPortDef->bPopulated      = OMX_TRUE;
            if(m_useZeroCopyBuffer == OMX_TRUE)
            {
                                        ...
                                        OMX_IN    OMX_U32                nSizeBytes)
{
    ...
    logv("(f:%s, l:%d) nPortIndex[%d], nSizeBytes[%d]",
...
//        return OMX_ErrorIncorrectStateOperation;
    ...
    if (nPortIndex == m_sInPortDefType.nPortIndex)
    {
        logv("allocate_buffer, m_sInPortDefType.nPortIndex[%d]", (int)m_sInPortDefType.nPortIndex);
        pthread_mutex_lock(&m_inBufMutex);
    ...
    else
    {
        ...
        m_useZeroCopyBuffer = OMX_FALSE;//not use zero copy
                                    ...
                                    OMX_IN  OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    logv("(f:%s, l:%d) nPortIndex = %d, pBufferHdr = %p, m_state=0x%x",
    ...
    else if (nPortIndex == m_sOutPortDefType.nPortIndex)
    {
        ...
        if(m_sOutBufList.nAllocSize == 0)
        {
            ...
            if(m_useAndroidBuffer == OMX_TRUE && m_useZeroCopyBuffer == OMX_TRUE)
            {
                //* free all gpu buffer
                omxFreeOutputBuffer(this);
            }
                                             ...
                                             OMX_IN  OMX_U32        uIndex)
{
    ...
    if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.mjpeg", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.mjpeg", OMX_MAX_STRINGNAME_SIZE);
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.mpeg1", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.mpeg1", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.mpeg2", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.mpeg2", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.msmpeg4v1", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.msmpeg4v1", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.msmpeg4v2", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.msmpeg4v2", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    /*
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.msmpeg4v3", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.msmpeg4v3", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.divx4", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.divx4", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.rx", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.rx", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.avs", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.avs", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    */
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.divx", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.divx", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.xvid", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.xvid", OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n", pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.s263", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.s263",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.rxg2", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.rxg2",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.wmv1", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.wmv1",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.wmv2", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.wmv2",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.vp6", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.vp6",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
    ...
    else if(!strncmp((char*)m_cName, "OMX.allwinner.video.decoder.vp9", OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.vp9",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
             ...
             OMX_MAX_STRINGNAME_SIZE))
    {
        if((0 == uIndex) && pRole)
        {
            strncpy((char *)pRole, "video_decoder.hevc",OMX_MAX_STRINGNAME_SIZE);
            logv("component_role_enum: pRole %s\n",pRole);
        }
        else
        {
...
static inline void setStateIdle(omx_vdec* pSelf)
{
    ...
    else
    {
        ...
        if (pSelf->m_state == OMX_StatePause || pSelf->m_state == OMX_StateExecuting)
        {
            ...
            if(pSelf->m_useZeroCopyBuffer == OMX_TRUE)
            {
                OMX_U32 i;
                for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
                {
        ...
        else    //OMX_StateLoaded -> OMX_StateIdle
        {
            ...
            {
                pSelf->m_useZeroCopyBuffer = OMX_TRUE;
                CdcMessage msg;
                memset(&msg, 0, sizeof(CdcMessage));
    ...
    if (cmddata == 0x1 || (OMX_S32)cmddata == -1)
    {
        ...
        if(pSelf->m_useZeroCopyBuffer == OMX_TRUE)
        {
            ...
            for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
            {
                ...
                {
                    logw("** return pic when flush,i[%lu],pPic[%p]",
...
int vdecoderPrepare(omx_vdec* pSelf)
{
    ...
    logd("m_useZeroCopyBuffer = %d, mExtraOutBufferNum = %d, m_useAndroid = %d",
        pSelf->m_useZeroCopyBuffer,pSelf->mExtraOutBufferNum, pSelf->m_useAndroidBuffer);
        ...
        && pSelf->mExtraOutBufferNum == 0)
    {
        ...
        else
        {
            ...
            SemTimedWait(&pSelf->m_semExtraOutBufferNum, 1000);
            logd(" wait for m_semExtraOutBufferNum done, mExtraOutBufferNum = %d",
...
static OMX_BUFFERHEADERTYPE* drainOutBufferInterlace(omx_vdec* pSelf)
{
    ...
    logv("drainOutBufferInterlace pSelf->mDiOutBufferInfo.nValidSize = %ld",
...
static OMX_BUFFERHEADERTYPE* drainOutBufferProgressive(omx_vdec* pSelf)
{
    ...
    else
    {
        ...
        if(pSelf->m_sInPortDefType.format.video.nFrameWidth == WIDTH_DEFAULT||
            pSelf->m_sInPortDefType.format.video.nFrameHeight == HEIGHT_DEFAULT)
        {
            pSelf->m_sInPortDefType.format.video.nFrameHeight = nPicRealHeight;
            pSelf->m_sInPortDefType.format.video.nFrameWidth  = nPicRealWidth;
        }
...
void OmxDrainVideoPictureToOutputBuffer(omx_vdec* pSelf)
{
    ...
    CdcMessage msg;
    memset(&msg, 0, sizeof(CdcMessage));
    msg.messageId = RENDER_THREAD_CMD_CONTINUE;
    CdcMessageQueuePostMessage(pSelf->mqRenderThread, &msg);
...
int initPartialGpuBuffer(char* pDataBuf, omx_vdec* pSelf)
{
    if(pSelf->nActualColorFormat == PIXEL_FORMAT_NV21)
    {
        //* Y
        int nRealHeight = pSelf->m_sOutPortDefType.format.video.nFrameHeight;
        int nInitHeight = pSelf->mGpuAlignStride;
        int nSkipLen = pSelf->m_sOutPortDefType.format.video.nStride*
            (nRealHeight - nInitHeight);
        int nCpyLenY = pSelf->m_sOutPortDefType.format.video.nStride*nInitHeight;
        memset(pDataBuf+nSkipLen, 0x10, nCpyLenY);

        //*UV
        nSkipLen += nCpyLenY;
        nSkipLen += (pSelf->m_sOutPortDefType.format.video.nStride)*
            (nRealHeight/2 - nInitHeight/2);
        int nCpyLenUV = (pSelf->m_sOutPortDefType.format.video.nStride)*(nInitHeight/2);
        memset(pDataBuf+nSkipLen, 0x80, nCpyLenUV);
    }
    else if(pSelf->nActualColorFormat == PIXEL_FORMAT_YV12)
    {
        //* Y
        int nRealHeight = pSelf->m_sOutPortDefType.format.video.nFrameHeight;
        int nInitHeight = pSelf->mGpuAlignStride;
        ...
        //*U
        nSkipLen += nCpyLenY;
        nSkipLen += (pSelf->m_sOutPortDefType.format.video.nStride/2)*
            (nRealHeight/2 - nInitHeight/2);
        int nCpyLenU = (pSelf->m_sOutPortDefType.format.video.nStride/2)*
            (nInitHeight/2);
        ...
        nSkipLen += (pSelf->m_sOutPortDefType.format.video.nStride/2)*
            (nRealHeight/2 - nInitHeight/2);
        int nCpyLenV = (pSelf->m_sOutPortDefType.format.video.nStride/2)*
            (nInitHeight/2);
...
#error invalid GPU type config
        ...
        if(pSelf->mIonFd > 0)
        {
            ...
            if(pSelf->mIsSecureVideoFlag)
                nTEEaddress = CdcIonGetTEEAdr(pSelf->mIonFd, (uintptr_t)handle_ion);
        }
        else
        {
        ...
        if(!pSelf->mIsSecureVideoFlag)
        {
            ...
            if(pSelf->m_streamInfo.eCodecFormat == VIDEO_CODEC_FORMAT_H265)
            {
                initPartialGpuBuffer((char*)dst,pSelf);
            }
        }
        ...
        if(!pSelf->mIsSecureVideoFlag)
            ...
            pSelf->mOutputBufferInfo[i].pBufVirAddr  = (char*)nTEEaddress;
...
static inline int OmxGetVideoFbmBufInfo(omx_vdec* pSelf)
{
    ...
    if(pFbmBufInfo != NULL)
    {
        ...
        pSelf->nActualColorFormat = (OMX_U32)pFbmBufInfo->ePixelFormat;
        ...
        if(pSelf->m_sInPortDefType.format.video.nFrameWidth == WIDTH_DEFAULT||
            pSelf->m_sInPortDefType.format.video.nFrameHeight == HEIGHT_DEFAULT)
        {
            pSelf->m_sInPortDefType.format.video.nFrameHeight = pFbmBufInfo->nBufHeight;
            pSelf->m_sInPortDefType.format.video.nFrameWidth  = pFbmBufInfo->nBufWidth;
        }
    ...
    else
    {
        ...
        if(pOutBufHdr != NULL)
        {
               ...
               && pSelf->mSetToDecoderBufferNum >= (OMX_U32)pSelf->mOutBufferNumDecoderNeed)
            {
                ...
                {
                    loge(" the mOutBufferKeepInOmx is overflow, i = %lu",i);
                    return -1;
                }
...
static inline int OmxSetOutEos(omx_vdec* pSelf)
{
    ...
    // to determine whether a playback is finished.
    pthread_mutex_lock(&pSelf->m_outBufMutex);
    logv("*** pSelf->m_sOutBufList.nSizeOfList = %lu",pSelf->m_sOutBufList.nSizeOfList);
    if(pSelf->m_sOutBufList.nSizeOfList > 0)
    {
        while (pSelf->m_sOutBufList.nSizeOfList > 0)
        {
...
static void* RenderThread(void* pThreadData)
{
    ...
    while (1)
    {
         if(CdcMessageQueueTryGetMessage(pSelf->mqRenderThread, &msg, 5) == 0)
        {
            ...
            (bPause == OMX_FALSE) || pSelf->bFlushAllFrameFlag)
        {
            ...
            int validPicNum = ValidPictureNum(pSelf->m_decoder, 0);
            logv("valid picture num[%d]",validPicNum);
            ...
            //*6. process the eos
            logv("LINE %d, nVdrvNotifyEosFlag %d", __LINE__, pSelf->mVdrvNotifyEosFlag);
            if(pSelf->mVdrvNotifyEosFlag && validPicNum == 0)
            {
                logd("validpicnum = %d, outlist = %d",
                    validPicNum, pSelf->m_sOutBufList.nSizeOfList);
                OmxSetOutEos(pSelf);
            }
            if(pSelf->bFlushAllFrameFlag && validPicNum == 0)
            {
...
process_vdrv_cmd:
        ...
        else
        {
            if(pSelf->mResolutionChangeFlag==OMX_TRUE)
            {
                ...
                logd("(f:%s, l:%d) wait for m_semFlushAllFrame", __FUNCTION__, __LINE__);
                SemTimedWait(&pSelf->m_semFlushAllFrame, -1);
                logd("(f:%s, l:%d) wait for m_semFlushAllFrame done!",
                    __FUNCTION__, __LINE__);
