...
#ifdef CONF_OMX_USE_SHARE_BUFFER
    bUseShareBuffer           = OMX_TRUE;
#else
    bUseShareBuffer           = OMX_FALSE;
#endif

    logd("gqy====bUseShareBuffer = %d", bUseShareBuffer);
...
static inline void controlFillBuf(omx_vdec* pSelf, OMX_U32 cmddata)
{
    ...
    {
        logd("gqy=== cache lis size: %d, allcoSize: %d, writepos: %d, readpos:%d",
...
static inline int vdecoderPrepare(omx_vdec* pSelf)
{
    ...
    m_videoConfig.nDecodeSmoothFrameBufferNum       = NUM_OF_PICTURES_FOR_EXTRA_SMOOTH + OMX_CACHE_BUFF_NUM;
...
void drainPicture(omx_vdec* pSelf)
{
    ...
    logv("****FillBufferDone is called, poUtBufHdr[%p],nSizeOfList[%d], pts[%lld],\
                                                    ...
                                                    OMX_BOOL mInitBufferFlag)
{
    ...
    if(mInitBufferFlag == OMX_TRUE)
    {
        ...
        logv("gqy=====nVirAddress = %p", nVirAddress);
...
static inline int OmxGetVideoFbmBufInfo(omx_vdec* pSelf)
{
    ...
    if(pFbmBufInfo != NULL)
    {
        ...
        pSelf->m_sOutPortDefType.format.video.nStride = (pFbmBufInfo->nBufWidth + 15)/16*16;
