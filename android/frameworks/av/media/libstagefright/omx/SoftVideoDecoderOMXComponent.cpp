...
void SoftVideoDecoderOMXComponent::updatePortDefinitions(bool updateCrop, bool updateInputSize) {
    ...
    outDef->format.video.nFrameWidth = isVp9CodingType?((outputBufferWidth()+31) &~ 31):outputBufferWidth();
    outDef->format.video.nFrameHeight = isVp9CodingType?((outputBufferHeight()+31) &~ 31):outputBufferHeight();
    ...
    //ALOGD("=== isVp9CodingType: %d, nBufferSize: %d", isVp9CodingType, outDef->nBufferSize);
...
void SoftVideoDecoderOMXComponent::handlePortSettingsChange(
    ...
    *portWillReset = false;
    ...
    if(isVp9CodingType)
    {
        uint32_t nStride = (def->format.video.nStride + 31) &~ 31;
        uint32_t nSliceHeight = (def->format.video.nSliceHeight + 31) &~ 31;
        if(nStride != def->format.video.nFrameWidth ||
            nSliceHeight != def->format.video.nFrameHeight)
        {
            strideChanged = true;
        }
    }
...
void SoftVideoDecoderOMXComponent::copyYV12FrameToOutputBuffer(
    ...
    size_t dstYStride = isVp9CodingType?((outputBufferWidth()+31) &~ 31):outputBufferWidth();
    size_t dstUVStride = dstYStride / 2;
    size_t dstHeight = isVp9CodingType?((outputBufferHeight()+31) &~ 31):outputBufferHeight();
    uint8_t *dstStart = dst;
    //ALOGD("==== mIsAdaptive: %d, dstYStride: %zu, dstHeight: %zu", mIsAdaptive, dstYStride, dstHeight);
...
OMX_ERRORTYPE SoftVideoDecoderOMXComponent::internalGetParameter(
    ...
    switch (index) {
        case OMX_IndexParamVideoPortFormat:
        {
            ...
            if(OMX_VIDEO_CodingVP9 == formatParams->eCompressionFormat)
            {
                ALOGD("===is vp9coding");
                isVp9CodingType = true;
            }
        ...
        case OMX_IndexParamPortDefinition:
        {
            ...
            if (newWidth != oldWidth || newHeight != oldHeight) {
                ...
                if (outputPort) {
                    ...
                    if(isVp9CodingType)
                    {
                        newParams->format.video.nFrameWidth = def->format.video.nFrameWidth;
                        newParams->format.video.nFrameHeight = def->format.video.nFrameHeight;
                    }
                    newParams->nBufferSize = def->nBufferSize;
                    //ALOGD("==== newParams->nBufferSize : %d", newParams->nBufferSize );
                } else {
