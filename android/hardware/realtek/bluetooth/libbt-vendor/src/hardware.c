...
static int osi_rand(void)
{
  int rand;
  int rand_fd = open(RANDOM_PATH, O_RDONLY);

  if (rand_fd == -1) {
    printf("%s can't open rand fd %s: %s ", __func__, RANDOM_PATH,
              strerror(errno));
    assert(0);
  }
...
static int getmacaddr(unsigned char * addr)
{
    ...
    static const char PERSIST_BDADDR_PROPERTY[] = "persist.service.bdroid.bdaddr";
    uint8_t bd_addr[BD_ADDR_LEN];
    char bd_str[] = "xx:xx:xx:xx:xx:xx";
    int i = 0;

    if ((vnd_local_bd_addr[0] == 0) && (vnd_local_bd_addr[1] == 0)
            && (vnd_local_bd_addr[2] == 0) && (vnd_local_bd_addr[3] == 0)
            && (vnd_local_bd_addr[4] == 0) && (vnd_local_bd_addr[5] == 0)) {
        return -1;
    } else {
        if ((vnd_local_bd_addr[0] == 0x22) && (vnd_local_bd_addr[1] == 0x22)) {
            /* Android HIDL use the pseudo rand() genarate bdaddr, this would
               lead the same macaddr for all machines. Here we re-genarate the
               baaddr use true random method */
            bd_addr[0] = 0x22;
            bd_addr[1] = 0x23;
            /* No autogen BDA. Generate one now. */
            bd_addr[2] = (uint8_t)osi_rand();
            bd_addr[3] = (uint8_t)osi_rand();
            bd_addr[4] = (uint8_t)osi_rand();
            bd_addr[5] = (uint8_t)osi_rand();
            /* Convert to ascii, and store as a persistent property */
            sprintf(bd_str, "%02x:%02x:%02x:%02x:%02x:%02x", bd_addr[0], bd_addr[1], bd_addr[2],
                    bd_addr[3], bd_addr[4], bd_addr[5]);

            if (property_set(PERSIST_BDADDR_PROPERTY, (char*)bd_str) < 0) {
                ALOGE("%s: Failed to set random BDA in prop %s", __func__,
                        PERSIST_BDADDR_PROPERTY);
            }
        } else {
            memcpy(bd_addr, vnd_local_bd_addr, 6);
        }

        for (i = 0; i < 6; i++) {
            addr[5-i] = (unsigned char)bd_addr[i];
        }
