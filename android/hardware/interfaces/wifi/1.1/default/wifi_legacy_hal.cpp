...
wifi_error WifiLegacyHal::initialize() {
  ...
  if (strcmp(wifi_vendor_name,"broadcom") == 0) {
     lib_handle = dlopen("libwifi-hal-bcm.so",RTLD_NOW);
  } else if (strcmp(wifi_vendor_name,"realtek") == 0) {
     lib_handle = dlopen("libwifi-hal-rtk.so",RTLD_NOW);
  } else if (strcmp(wifi_vendor_name,"qualcomm") == 0) {
     lib_handle = dlopen("libwifi-hal-qcom.so",RTLD_NOW);
  } else {
     LOG(ERROR) << "unknow wifi vendor name,return!!!";
     return WIFI_ERROR_UNKNOWN;
  }
  init_wifi_vendor_hal_func_table_P = (wifi_error (*)(wifi_hal_fn *))dlsym(lib_handle,"init_wifi_vendor_hal_func_table"); 
  wifi_error status = init_wifi_vendor_hal_func_table_P(&global_func_table_);
