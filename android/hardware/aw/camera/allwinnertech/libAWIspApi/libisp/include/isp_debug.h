...
#define ISP_WARN(fmt,arg...)  \
    LOG_PRI(ANDROID_LOG_WARN, "ISP", "<%s:%u>: " fmt, __FUNCTION__, __LINE__, ##arg)
#define ISP_PRINT(fmt,arg...)  \
    LOG_PRI(ANDROID_LOG_DEBUG, "ISP", "<%s:%u>: " fmt, __FUNCTION__, __LINE__, ##arg)
#define ISP_ERR(fmt, arg...)  \
    LOG_PRI(ANDROID_LOG_ERROR, "ISP", "<%s:%u>: " fmt, __FUNCTION__, __LINE__, ##arg)
...
#define ISP_DEV_LOG(flag, msg...)\
	do {\
		...
		LOG_PRI(ANDROID_LOG_DEBUG, "[ISP_DEV_LOG]: ", ##msg);\
	} while (0);\

#define ISP_LIB_LOG(flag, msg...)\
	do {\
		if (isp_lib_log_param & flag)\
		LOG_PRI(ANDROID_LOG_DEBUG, "[ISP_LIB_LOG]: ", ##msg);\
	} while (0);\
...
#define ISP_CFG_LOG(flag, msg...)\
	do {\
		...
		LOG_PRI(ANDROID_LOG_DEBUG, "[ISP_CFG_LOG]: ", ##msg);\
	} while (0);\
