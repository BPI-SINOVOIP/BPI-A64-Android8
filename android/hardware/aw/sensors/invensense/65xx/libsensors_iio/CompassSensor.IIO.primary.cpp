...
#define MAG_DATA_CONVERSION(value) (float)((float)(((int)value)*(0.6f)))
...
#elif defined COMPASS_AK09911
#   warning "Invensense compass cal with AKM09911 IIO on primary bus"
#   define USE_MPL_COMPASS_HAL          (1)
#   define COMPASS_NAME                 "akm09911"
                    ...
                    mCoilsResetFd(0)
{
	...
	mPendingEvent.type = SENSOR_TYPE_GEOMAGNETIC_FIELD;
...
int CompassSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    /*LOGI("get one sample of AMI IIO data with size: %d", rsize);
    LOGE("compass x/y/z: %d/%d/%d ,mEnable : %d", *((short *) (rdata + 0)),
        *((short *) (rdata + 2)), *((short *) (rdata + 4)),mEnable);*/

    if (mEnable) {
        for (i = 0; i < 3; i++) {
            mCache[i] = *((short *) (rdata + i * 2));
        }
        timestamp =*((long long *) (rdata + 8 * mEnable));
		mPendingEvent.timestamp = timestamp;
		mPendingEvent.magnetic.x = MAG_DATA_CONVERSION(mCache[0]);
		mPendingEvent.magnetic.y = MAG_DATA_CONVERSION(mCache[1]);
		mPendingEvent.magnetic.z = MAG_DATA_CONVERSION(mCache[2]);
		mPendingEvent.magnetic.status = 1;
		*data++ = mPendingEvent;
		return 1;
    }
	return 0;
