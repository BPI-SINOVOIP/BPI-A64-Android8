...
int MPLSensorSysApi::setBiasUpdateFunc(int f)
{
    FUNC_LOG;
...
int MPLSensorSysApi::setSensors(int s)
{
    FUNC_LOG;
    int rv = INV_SUCCESS;
...
int MPLSensorSysApi::getSensors(int* s)
{
    FUNC_LOG;
    int rv = INV_SUCCESS;
    pthread_mutex_lock(&mMplMutex);
...
int MPLSensorSysApi::selfTest()
{
    ...
    do {
        ...
        LOGI("Bias: X:Y:Z (%d, %d, %d)", atoi(x), atoi(y), atoi(z));
        if ( atoi(result) ) {
