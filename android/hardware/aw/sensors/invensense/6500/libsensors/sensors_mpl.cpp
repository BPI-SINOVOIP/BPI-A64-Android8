...
#endif
#ifdef ALL_WINNER_LIGHT_SENSOR
#define LOCAL_SENSORS (MPLSensor::NumSensors+3)
#endif
...
int sensors_poll_context_t::activate(int handle, int enabled) {
    ...
    } else if (handle == ID_PX || handle == ID_PX_W) {
...
int sensors_poll_context_t::setDelay(int handle, int64_t ns)
{
    ...
    int err;
    if (handle == ID_L) {
        err = mLigntSensor->setEnable(handle, ns);
    } else if (handle == ID_PX || handle == ID_PX_W) {
        err = mProximitySensor->setDelay(handle, ns);
    } else {
        err = mSensor->setDelay(handle, ns);
    }
    return err;
    #else
