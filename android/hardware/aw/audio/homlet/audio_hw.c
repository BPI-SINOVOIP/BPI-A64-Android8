...
INIT_END:
    ...
    ALOGD("[0] flag_out %d, flag_out_active %d;[1] flag_out%d, flag_out_active %d", adev->dev_manager[0].flag_out, adev->dev_manager[0].flag_out_active,
...
static int start_output_stream(struct sunxi_stream_out *out)
{
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) // 10-16 modify
...
//	if ((adev->mode == AUDIO_MODE_IN_CALL&&adev->out_device==AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET)||adev->mode == AUDIO_MODE_MODE_FACTORY_TEST || adev->mode == AUDIO_MODE_FM)
    ...
    for (index = MAX_AUDIO_DEVICES; index >= 0; index--)
    {
            ...
            && out->multi_pcm[index] != NULL)
        {
            ...
            if(ret!=0){
                ALOGE("##############out_write()  Warning:write fail, reopen it ret = %d ,strerror(errno) %s #######################", ret, strerror(errno));
                do_output_standby(out);
                usleep(30000);
                break;
            }
...
reStart:
        ...
        if (cerr_in > 0) {
            ...
            if (audio_data->usb_pcm) {
                ...
                ret = pcm_read(audio_data->usb_pcm, buffer, size);
                if (usb_data_cnt > 0) {
                     ...
                     hw_device_t** device)
{
    ...
    *device = &adev->hw_device.common;
    ...
    ALOGD("OK:  ****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);
    return 0;
