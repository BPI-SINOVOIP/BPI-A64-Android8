...
/**
 * A {@link FastbootDeviceFlasher} for flashing LG Hammerhead devices
 */
public class HammerheadDeviceFlasher extends FastbootDeviceFlasher {
    /** The minimum bootloader version that we support flashing to/from */
    static final String MIN_BOOTLOADER_VERSION = "HHZ10a";
    ...
    protected boolean checkAndFlashBootloader(ITestDevice device, IDeviceBuildInfo deviceBuild)
        ...
        if (!newVersion.matches("^HHZ\\d\\d[a-z]?$")) {
            throw new TargetSetupError(String.format("TF flasher only supports devices with " +
                    "bootloader version HHZ[0-9][0-9][a-z].  Current version, %s, should " +
