...
import com.google.android.tradefed.build.AppLaunchControlProvider;
...
/**
 * An {@link AppLaunchControlProvider} that only returns a build to test if Monkey Post
 ...
 * <p/>
 * TODO: consolidate logic with {@link AppWithDeviceMonkeyBuildProvider}
 */
@OptionClass(alias = "appmonkey-build")
public class AppMonkeyBuildProvider extends AppLaunchControlProvider
    ...
    @Option(name = "skip-count-check", description =
            "whether to skip the run count check and act like AppLaunchControlProvider")
    private Boolean mSkipCountCheck = false;

    @Option(name = "run-limit", description = "target number of runs")
    private int mRunLimit = 20;

    @Option(name = "prod-type", description =
            "Product type name as recognized by counting service." +
            "Default will be device variant")
    ...
    public IBuildInfo getBuild(ITestDevice device) throws BuildRetrievalError,
        ...
        } else if (needMoreMonkeyRuns(mRemoteBuild, device)) {
    ...
    boolean needMoreMonkeyRuns(RemoteBuildInfo build, ITestDevice device)
        throws BuildRetrievalError, DeviceNotAvailableException {
        if (mSkipCountCheck) return true;

        try {
            int nRuns = getRunCount(build, device);
            if (nRuns < mRunLimit) {
                // THIS IS A HACK: reset build in LaunchControl so that the next invocation will
                // also pick up this build.  It is an unfortunate side-effect that this guarantees
                // extra runs in any case where we have more than one invocation running at the
                // same time.
                resetTestBuild(build.getBuildId());
                return true;
            }
        } catch (BuildRetrievalError e) {
            // failed to get count, reset the build so next attempt will try again
            resetTestBuild(build.getBuildId());
            throw e;
     ...
     */
    int getRunCount(RemoteBuildInfo build, ITestDevice device) throws BuildRetrievalError,
            DeviceNotAvailableException {
        if (mProdType == null) {
            mProdType = device.getProductVariant();
            if (mProdType == null) {
                throw new BuildRetrievalError("Could not get device product variant");
            }
        }

        final MultiMap<String, String> params = new MultiMap<String, String>();
        params.put(PARAM_KEY, getTestTag());
        params.put(PARAM_PRODUCT, mProdType);
        params.put(PARAM_APP_BUILD_ID, build.getBuildId());
        params.put(PARAM_BUILD_ID, device.getBuildId());
