...
/**
 * Unit tests for {@link DeviceWithAppLaunchControlProvider}.
 */
public class DeviceWithAppLaunchControlProviderTest extends TestCase {
    ...
    private DeviceWithAppLaunchControlProvider mProvider;
    ...
    protected void setUp() throws Exception {
        ...
        mProvider = new DeviceWithAppLaunchControlProvider() {
            @Override
        ...
        mProvider.setAppBranch(APP_BRANCH);
        mProvider.setAppBuildFlavor(APP_BUILD_FLAVOR);
    ...
    /**
     * Test normal case for {@link DeviceWithAppLaunchControlProvider#getBuild()}.
     */
    ...
    /**
     * Test {@link DeviceWithAppLaunchControlProvider#getBuild()} when there is no App build
     * available for testing.
     */
    @SuppressWarnings("unchecked")
    public void testGetBuild_noAppBuild() throws IOException, DataSizeException,
            BuildRetrievalError {
        EasyMock.expect(mMockHttpHelper.buildUrl((String)EasyMock.anyObject(),
                (MultiMap<String, String>)EasyMock.anyObject())).andStubReturn("");
        EasyMock.expect(mMockHttpHelper.doGetWithRetry((String)EasyMock.anyObject())).andReturn(
    ...
    /**
     * Test {@link DeviceWithAppLaunchControlProvider#getBuild()} when there is a app build, but no
     * device build
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Test {@link DeviceWithAppLaunchControlProvider#getBuild()} when download of app build fails.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Test {@link DeviceLaunchControlProvider#getBuild()} when download of device build fails.
     */
    @SuppressWarnings("unchecked")
    public void testGetBuild_appDownloadFailed() throws IOException, DataSizeException,
            BuildRetrievalError {
        EasyMock.expect(mMockHttpHelper.buildUrl((String)EasyMock.anyObject(),
                (MultiMap<String, String>)EasyMock.anyObject())).andReturn("");
        EasyMock.expect(mMockHttpHelper.buildUrl((String)EasyMock.anyObject(),
        ...
        EasyMock.expect(mMockHttpHelper.buildUrl((String)EasyMock.anyObject(),
                (MultiMap<String, String>)EasyMock.anyObject())).andReturn("");
        // expect download of device image, and userdata
        EasyMock.expect(mMockDownloader.downloadFile(DEVICE_IMAGE_PATH)).andReturn(mStubImageFile);
        EasyMock.expect(mMockDownloader.downloadFile(DEVICE_TEST_PATH)).andReturn(mMockFile);
