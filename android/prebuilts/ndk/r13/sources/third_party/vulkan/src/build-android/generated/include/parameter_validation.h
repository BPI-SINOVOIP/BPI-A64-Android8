    ...
    VkInstance*                                 pInstance)
{
    ...
    if (pCreateInfo != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkCreateInstance", "pCreateInfo->pApplicationInfo->pNext", NULL, pCreateInfo->pApplicationInfo->pNext, 0, NULL);
        }
    ...
    VkDevice*                                   pDevice)
{
    ...
    if (pCreateInfo != NULL)
    {
        ...
        if (pCreateInfo->pQueueCreateInfos != NULL)
        {
            ...
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateDevice", "pCreateInfo->pQueueCreateInfos[i].pNext", NULL, pCreateInfo->pQueueCreateInfos[queueCreateInfoIndex].pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateDevice", "pCreateInfo->pQueueCreateInfos[i].flags", pCreateInfo->pQueueCreateInfos[queueCreateInfoIndex].flags);

                skipCall |= validate_array(report_data, "vkCreateDevice", "pCreateInfo->pQueueCreateInfos[i].queueCount", "pCreateInfo->pQueueCreateInfos[i].pQueuePriorities", pCreateInfo->pQueueCreateInfos[queueCreateInfoIndex].queueCount, pCreateInfo->pQueueCreateInfos[queueCreateInfoIndex].pQueuePriorities, true, true);
            }
        }
    ...
    VkFence                                     fence)
{
    ...
    if (pSubmits != NULL)
    {
        for (uint32_t submitIndex = 0; submitIndex < submitCount; ++submitIndex)
        {
            ...
            skipCall |= validate_array(report_data, "vkQueueSubmit", "pSubmits[i].waitSemaphoreCount", "pSubmits[i].pWaitSemaphores", pSubmits[submitIndex].waitSemaphoreCount, pSubmits[submitIndex].pWaitSemaphores, false, true);

            skipCall |= validate_flags_array(report_data, "vkQueueSubmit", "pSubmits[i].waitSemaphoreCount", "pSubmits[i].pWaitDstStageMask", "VkPipelineStageFlagBits", AllVkPipelineStageFlagBits, pSubmits[submitIndex].waitSemaphoreCount, pSubmits[submitIndex].pWaitDstStageMask, false, true);

            skipCall |= validate_array(report_data, "vkQueueSubmit", "pSubmits[i].commandBufferCount", "pSubmits[i].pCommandBuffers", pSubmits[submitIndex].commandBufferCount, pSubmits[submitIndex].pCommandBuffers, false, true);

            skipCall |= validate_array(report_data, "vkQueueSubmit", "pSubmits[i].signalSemaphoreCount", "pSubmits[i].pSignalSemaphores", pSubmits[submitIndex].signalSemaphoreCount, pSubmits[submitIndex].pSignalSemaphores, false, true);
        }
    }
    ...
    const VkMappedMemoryRange*                  pMemoryRanges)
{
    ...
    if (pMemoryRanges != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkFlushMappedMemoryRanges", "pMemoryRanges[i].pNext", NULL, pMemoryRanges[memoryRangeIndex].pNext, 0, NULL);

            skipCall |= validate_required_handle(report_data, "vkFlushMappedMemoryRanges", "pMemoryRanges[i].memory", pMemoryRanges[memoryRangeIndex].memory);
        }
    }
    ...
    const VkMappedMemoryRange*                  pMemoryRanges)
{
    ...
    if (pMemoryRanges != NULL)
    {
        for (uint32_t memoryRangeIndex = 0; memoryRangeIndex < memoryRangeCount; ++memoryRangeIndex)
        {
            skipCall |= validate_struct_pnext(report_data, "vkInvalidateMappedMemoryRanges", "pMemoryRanges[i].pNext", NULL, pMemoryRanges[memoryRangeIndex].pNext, 0, NULL);

            skipCall |= validate_required_handle(report_data, "vkInvalidateMappedMemoryRanges", "pMemoryRanges[i].memory", pMemoryRanges[memoryRangeIndex].memory);
        }
    }
    ...
    VkFence                                     fence)
{
    ...
    if (pBindInfo != NULL)
    {
        for (uint32_t bindInfoIndex = 0; bindInfoIndex < bindInfoCount; ++bindInfoIndex)
        {
            ...
            skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].waitSemaphoreCount", "pBindInfo[i].pWaitSemaphores", pBindInfo[bindInfoIndex].waitSemaphoreCount, pBindInfo[bindInfoIndex].pWaitSemaphores, false, true);

            skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].bufferBindCount", "pBindInfo[i].pBufferBinds", pBindInfo[bindInfoIndex].bufferBindCount, pBindInfo[bindInfoIndex].pBufferBinds, false, true);
            ...
            if (pBindInfo[bindInfoIndex].pBufferBinds != NULL)
            {
                ...
                {
                    skipCall |= validate_required_handle(report_data, "vkQueueBindSparse", "pBindInfo[i].pBufferBinds[i].buffer", pBindInfo[bindInfoIndex].pBufferBinds[bufferBindIndex].buffer);

                    skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].pBufferBinds[i].bindCount", "pBindInfo[i].pBufferBinds[i].pBinds", pBindInfo[bindInfoIndex].pBufferBinds[bufferBindIndex].bindCount, pBindInfo[bindInfoIndex].pBufferBinds[bufferBindIndex].pBinds, true, true);
                    ...
                    if (pBindInfo[bindInfoIndex].pBufferBinds[bufferBindIndex].pBinds != NULL)
                    {
                        ...
                        {
                            skipCall |= validate_flags(report_data, "vkQueueBindSparse", "pBindInfo[i].pBufferBinds[i].pBinds[i].flags", "VkSparseMemoryBindFlagBits", AllVkSparseMemoryBindFlagBits, pBindInfo[bindInfoIndex].pBufferBinds[bufferBindIndex].pBinds[bindIndex].flags, false);
                        }
                    }
                }
            }

            skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].imageOpaqueBindCount", "pBindInfo[i].pImageOpaqueBinds", pBindInfo[bindInfoIndex].imageOpaqueBindCount, pBindInfo[bindInfoIndex].pImageOpaqueBinds, false, true);
            ...
            if (pBindInfo[bindInfoIndex].pImageOpaqueBinds != NULL)
            {
                ...
                {
                    skipCall |= validate_required_handle(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageOpaqueBinds[i].image", pBindInfo[bindInfoIndex].pImageOpaqueBinds[imageOpaqueBindIndex].image);

                    skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageOpaqueBinds[i].bindCount", "pBindInfo[i].pImageOpaqueBinds[i].pBinds", pBindInfo[bindInfoIndex].pImageOpaqueBinds[imageOpaqueBindIndex].bindCount, pBindInfo[bindInfoIndex].pImageOpaqueBinds[imageOpaqueBindIndex].pBinds, true, true);
                    ...
                    if (pBindInfo[bindInfoIndex].pImageOpaqueBinds[imageOpaqueBindIndex].pBinds != NULL)
                    {
                        ...
                        {
                            skipCall |= validate_flags(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageOpaqueBinds[i].pBinds[i].flags", "VkSparseMemoryBindFlagBits", AllVkSparseMemoryBindFlagBits, pBindInfo[bindInfoIndex].pImageOpaqueBinds[imageOpaqueBindIndex].pBinds[bindIndex].flags, false);
                        }
                    }
                }
            }

            skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].imageBindCount", "pBindInfo[i].pImageBinds", pBindInfo[bindInfoIndex].imageBindCount, pBindInfo[bindInfoIndex].pImageBinds, false, true);
            ...
            if (pBindInfo[bindInfoIndex].pImageBinds != NULL)
            {
                ...
                {
                    skipCall |= validate_required_handle(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageBinds[i].image", pBindInfo[bindInfoIndex].pImageBinds[imageBindIndex].image);

                    skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageBinds[i].bindCount", "pBindInfo[i].pImageBinds[i].pBinds", pBindInfo[bindInfoIndex].pImageBinds[imageBindIndex].bindCount, pBindInfo[bindInfoIndex].pImageBinds[imageBindIndex].pBinds, true, true);
                    ...
                    if (pBindInfo[bindInfoIndex].pImageBinds[imageBindIndex].pBinds != NULL)
                    {
                        ...
                        {
                            skipCall |= validate_flags(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageBinds[i].pBinds[i].subresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pBindInfo[bindInfoIndex].pImageBinds[imageBindIndex].pBinds[bindIndex].subresource.aspectMask, true);

                            skipCall |= validate_flags(report_data, "vkQueueBindSparse", "pBindInfo[i].pImageBinds[i].pBinds[i].flags", "VkSparseMemoryBindFlagBits", AllVkSparseMemoryBindFlagBits, pBindInfo[bindInfoIndex].pImageBinds[imageBindIndex].pBinds[bindIndex].flags, false);
                        }
                    }
                }
            }

            skipCall |= validate_array(report_data, "vkQueueBindSparse", "pBindInfo[i].signalSemaphoreCount", "pBindInfo[i].pSignalSemaphores", pBindInfo[bindInfoIndex].signalSemaphoreCount, pBindInfo[bindInfoIndex].pSignalSemaphores, false, true);
        }
    }
    ...
    VkPipeline*                                 pPipelines)
{
    ...
    if (pCreateInfos != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pNext", NULL, pCreateInfos[createInfoIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].flags", "VkPipelineCreateFlagBits", AllVkPipelineCreateFlagBits, pCreateInfos[createInfoIndex].flags, false);

            skipCall |= validate_struct_type_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].stageCount", "pCreateInfos[i].pStages", "VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO", pCreateInfos[createInfoIndex].stageCount, pCreateInfos[createInfoIndex].pStages, VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, true, true);
            ...
            if (pCreateInfos[createInfoIndex].pStages != NULL)
            {
                ...
                {
                    skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pStages[i].pNext", NULL, pCreateInfos[createInfoIndex].pStages[stageIndex].pNext, 0, NULL);

                    skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pStages[i].flags", pCreateInfos[createInfoIndex].pStages[stageIndex].flags);

                    skipCall |= validate_required_handle(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pStages[i].module", pCreateInfos[createInfoIndex].pStages[stageIndex].module);

                    skipCall |= validate_required_pointer(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pStages[i].pName", pCreateInfos[createInfoIndex].pStages[stageIndex].pName);

                    if (pCreateInfos[createInfoIndex].pStages[stageIndex].pSpecializationInfo != NULL)
                    {
                        skipCall |= validate_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pStages[i].pSpecializationInfo->mapEntryCount", "pCreateInfos[i].pStages[i].pSpecializationInfo->pMapEntries", pCreateInfos[createInfoIndex].pStages[stageIndex].pSpecializationInfo->mapEntryCount, pCreateInfos[createInfoIndex].pStages[stageIndex].pSpecializationInfo->pMapEntries, false, true);
            ...
            skipCall |= validate_struct_type(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState", "VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO", pCreateInfos[createInfoIndex].pVertexInputState, VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, true);

            if (pCreateInfos[createInfoIndex].pVertexInputState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState->pNext", NULL, pCreateInfos[createInfoIndex].pVertexInputState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState->flags", pCreateInfos[createInfoIndex].pVertexInputState->flags);

                skipCall |= validate_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState->vertexBindingDescriptionCount", "pCreateInfos[i].pVertexInputState->pVertexBindingDescriptions", pCreateInfos[createInfoIndex].pVertexInputState->vertexBindingDescriptionCount, pCreateInfos[createInfoIndex].pVertexInputState->pVertexBindingDescriptions, false, true);
                ...
                if (pCreateInfos[createInfoIndex].pVertexInputState->pVertexBindingDescriptions != NULL)
                {
                    ...
                    {
                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState->pVertexBindingDescriptions[i].inputRate", "VkVertexInputRate", VK_VERTEX_INPUT_RATE_BEGIN_RANGE, VK_VERTEX_INPUT_RATE_END_RANGE, pCreateInfos[createInfoIndex].pVertexInputState->pVertexBindingDescriptions[vertexBindingDescriptionIndex].inputRate);
                    }
                }

                skipCall |= validate_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState->vertexAttributeDescriptionCount", "pCreateInfos[i].pVertexInputState->pVertexAttributeDescriptions", pCreateInfos[createInfoIndex].pVertexInputState->vertexAttributeDescriptionCount, pCreateInfos[createInfoIndex].pVertexInputState->pVertexAttributeDescriptions, false, true);
                ...
                if (pCreateInfos[createInfoIndex].pVertexInputState->pVertexAttributeDescriptions != NULL)
                {
                    ...
                    {
                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pVertexInputState->pVertexAttributeDescriptions[i].format", "VkFormat", VK_FORMAT_BEGIN_RANGE, VK_FORMAT_END_RANGE, pCreateInfos[createInfoIndex].pVertexInputState->pVertexAttributeDescriptions[vertexAttributeDescriptionIndex].format);
                    }
                }
            }

            skipCall |= validate_struct_type(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pInputAssemblyState", "VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO", pCreateInfos[createInfoIndex].pInputAssemblyState, VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO, true);

            if (pCreateInfos[createInfoIndex].pInputAssemblyState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pInputAssemblyState->pNext", NULL, pCreateInfos[createInfoIndex].pInputAssemblyState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pInputAssemblyState->flags", pCreateInfos[createInfoIndex].pInputAssemblyState->flags);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pInputAssemblyState->topology", "VkPrimitiveTopology", VK_PRIMITIVE_TOPOLOGY_BEGIN_RANGE, VK_PRIMITIVE_TOPOLOGY_END_RANGE, pCreateInfos[createInfoIndex].pInputAssemblyState->topology);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pInputAssemblyState->primitiveRestartEnable", pCreateInfos[createInfoIndex].pInputAssemblyState->primitiveRestartEnable);
            }

            if (pCreateInfos[createInfoIndex].pTessellationState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pTessellationState->pNext", NULL, pCreateInfos[createInfoIndex].pTessellationState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pTessellationState->flags", pCreateInfos[createInfoIndex].pTessellationState->flags);
            }

            if (pCreateInfos[createInfoIndex].pViewportState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pViewportState->pNext", NULL, pCreateInfos[createInfoIndex].pViewportState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pViewportState->flags", pCreateInfos[createInfoIndex].pViewportState->flags);
            }

            skipCall |= validate_struct_type(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState", "VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO", pCreateInfos[createInfoIndex].pRasterizationState, VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO, true);
            ...
            if (pCreateInfos[createInfoIndex].pRasterizationState != NULL)
            {
                ...
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->pNext", "VkPipelineRasterizationStateRasterizationOrderAMD", pCreateInfos[createInfoIndex].pRasterizationState->pNext, ARRAY_SIZE(allowedStructs), allowedStructs);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->flags", pCreateInfos[createInfoIndex].pRasterizationState->flags);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->depthClampEnable", pCreateInfos[createInfoIndex].pRasterizationState->depthClampEnable);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->rasterizerDiscardEnable", pCreateInfos[createInfoIndex].pRasterizationState->rasterizerDiscardEnable);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->polygonMode", "VkPolygonMode", VK_POLYGON_MODE_BEGIN_RANGE, VK_POLYGON_MODE_END_RANGE, pCreateInfos[createInfoIndex].pRasterizationState->polygonMode);

                skipCall |= validate_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->cullMode", "VkCullModeFlagBits", AllVkCullModeFlagBits, pCreateInfos[createInfoIndex].pRasterizationState->cullMode, false);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->frontFace", "VkFrontFace", VK_FRONT_FACE_BEGIN_RANGE, VK_FRONT_FACE_END_RANGE, pCreateInfos[createInfoIndex].pRasterizationState->frontFace);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pRasterizationState->depthBiasEnable", pCreateInfos[createInfoIndex].pRasterizationState->depthBiasEnable);
            }

            if (pCreateInfos[createInfoIndex].pMultisampleState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pMultisampleState->pNext", NULL, pCreateInfos[createInfoIndex].pMultisampleState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pMultisampleState->flags", pCreateInfos[createInfoIndex].pMultisampleState->flags);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pMultisampleState->sampleShadingEnable", pCreateInfos[createInfoIndex].pMultisampleState->sampleShadingEnable);

                skipCall |= validate_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pMultisampleState->rasterizationSamples", "pCreateInfos[i].pMultisampleState->pSampleMask", pCreateInfos[createInfoIndex].pMultisampleState->rasterizationSamples, pCreateInfos[createInfoIndex].pMultisampleState->pSampleMask, true, false);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pMultisampleState->alphaToCoverageEnable", pCreateInfos[createInfoIndex].pMultisampleState->alphaToCoverageEnable);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pMultisampleState->alphaToOneEnable", pCreateInfos[createInfoIndex].pMultisampleState->alphaToOneEnable);
            }

            if (pCreateInfos[createInfoIndex].pDepthStencilState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->pNext", NULL, pCreateInfos[createInfoIndex].pDepthStencilState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->flags", pCreateInfos[createInfoIndex].pDepthStencilState->flags);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->depthTestEnable", pCreateInfos[createInfoIndex].pDepthStencilState->depthTestEnable);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->depthWriteEnable", pCreateInfos[createInfoIndex].pDepthStencilState->depthWriteEnable);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->depthCompareOp", "VkCompareOp", VK_COMPARE_OP_BEGIN_RANGE, VK_COMPARE_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->depthCompareOp);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->depthBoundsTestEnable", pCreateInfos[createInfoIndex].pDepthStencilState->depthBoundsTestEnable);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->stencilTestEnable", pCreateInfos[createInfoIndex].pDepthStencilState->stencilTestEnable);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->front.failOp", "VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->front.failOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->front.passOp", "VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->front.passOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->front.depthFailOp", "VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->front.depthFailOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->front.compareOp", "VkCompareOp", VK_COMPARE_OP_BEGIN_RANGE, VK_COMPARE_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->front.compareOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->back.failOp", "VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->back.failOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->back.passOp", "VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->back.passOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->back.depthFailOp", "VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->back.depthFailOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDepthStencilState->back.compareOp", "VkCompareOp", VK_COMPARE_OP_BEGIN_RANGE, VK_COMPARE_OP_END_RANGE, pCreateInfos[createInfoIndex].pDepthStencilState->back.compareOp);
            }

            if (pCreateInfos[createInfoIndex].pColorBlendState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pNext", NULL, pCreateInfos[createInfoIndex].pColorBlendState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->flags", pCreateInfos[createInfoIndex].pColorBlendState->flags);

                skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->logicOpEnable", pCreateInfos[createInfoIndex].pColorBlendState->logicOpEnable);

                skipCall |= validate_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->attachmentCount", "pCreateInfos[i].pColorBlendState->pAttachments", pCreateInfos[createInfoIndex].pColorBlendState->attachmentCount, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments, false, true);

                if (pCreateInfos[createInfoIndex].pColorBlendState->pAttachments != NULL)
                {
                    for (uint32_t attachmentIndex = 0; attachmentIndex < pCreateInfos[createInfoIndex].pColorBlendState->attachmentCount; ++attachmentIndex)
                    {
                        skipCall |= validate_bool32(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].blendEnable", pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].blendEnable);

                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].srcColorBlendFactor", "VkBlendFactor", VK_BLEND_FACTOR_BEGIN_RANGE, VK_BLEND_FACTOR_END_RANGE, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].srcColorBlendFactor);

                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].dstColorBlendFactor", "VkBlendFactor", VK_BLEND_FACTOR_BEGIN_RANGE, VK_BLEND_FACTOR_END_RANGE, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].dstColorBlendFactor);

                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].colorBlendOp", "VkBlendOp", VK_BLEND_OP_BEGIN_RANGE, VK_BLEND_OP_END_RANGE, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].colorBlendOp);

                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].srcAlphaBlendFactor", "VkBlendFactor", VK_BLEND_FACTOR_BEGIN_RANGE, VK_BLEND_FACTOR_END_RANGE, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].srcAlphaBlendFactor);

                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].dstAlphaBlendFactor", "VkBlendFactor", VK_BLEND_FACTOR_BEGIN_RANGE, VK_BLEND_FACTOR_END_RANGE, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].dstAlphaBlendFactor);

                        skipCall |= validate_ranged_enum(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].alphaBlendOp", "VkBlendOp", VK_BLEND_OP_BEGIN_RANGE, VK_BLEND_OP_END_RANGE, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].alphaBlendOp);

                        skipCall |= validate_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pColorBlendState->pAttachments[i].colorWriteMask", "VkColorComponentFlagBits", AllVkColorComponentFlagBits, pCreateInfos[createInfoIndex].pColorBlendState->pAttachments[attachmentIndex].colorWriteMask, false);
                    }
                }
            }

            skipCall |= validate_struct_type(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDynamicState", "VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO", pCreateInfos[createInfoIndex].pDynamicState, VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO, false);

            if (pCreateInfos[createInfoIndex].pDynamicState != NULL)
            {
                skipCall |= validate_struct_pnext(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDynamicState->pNext", NULL, pCreateInfos[createInfoIndex].pDynamicState->pNext, 0, NULL);

                skipCall |= validate_reserved_flags(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDynamicState->flags", pCreateInfos[createInfoIndex].pDynamicState->flags);

                skipCall |= validate_ranged_enum_array(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].pDynamicState->dynamicStateCount", "pCreateInfos[i].pDynamicState->pDynamicStates", "VkDynamicState", VK_DYNAMIC_STATE_BEGIN_RANGE, VK_DYNAMIC_STATE_END_RANGE, pCreateInfos[createInfoIndex].pDynamicState->dynamicStateCount, pCreateInfos[createInfoIndex].pDynamicState->pDynamicStates, true, true);
            }

            skipCall |= validate_required_handle(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].layout", pCreateInfos[createInfoIndex].layout);

            skipCall |= validate_required_handle(report_data, "vkCreateGraphicsPipelines", "pCreateInfos[i].renderPass", pCreateInfos[createInfoIndex].renderPass);
        }
    }
    ...
    VkPipeline*                                 pPipelines)
{
    ...
    if (pCreateInfos != NULL)
    {
        for (uint32_t createInfoIndex = 0; createInfoIndex < createInfoCount; ++createInfoIndex)
        {
            skipCall |= validate_struct_pnext(report_data, "vkCreateComputePipelines", "pCreateInfos[i].pNext", NULL, pCreateInfos[createInfoIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCreateComputePipelines", "pCreateInfos[i].flags", "VkPipelineCreateFlagBits", AllVkPipelineCreateFlagBits, pCreateInfos[createInfoIndex].flags, false);

            skipCall |= validate_struct_type(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage", "VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO", &(pCreateInfos[createInfoIndex].stage), VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, false);

            skipCall |= validate_struct_pnext(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage.pNext", NULL, pCreateInfos[createInfoIndex].stage.pNext, 0, NULL);

            skipCall |= validate_reserved_flags(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage.flags", pCreateInfos[createInfoIndex].stage.flags);

            skipCall |= validate_required_handle(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage.module", pCreateInfos[createInfoIndex].stage.module);

            skipCall |= validate_required_pointer(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage.pName", pCreateInfos[createInfoIndex].stage.pName);

            if (pCreateInfos[createInfoIndex].stage.pSpecializationInfo != NULL)
            {
                skipCall |= validate_array(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage.pSpecializationInfo->mapEntryCount", "pCreateInfos[i].stage.pSpecializationInfo->pMapEntries", pCreateInfos[createInfoIndex].stage.pSpecializationInfo->mapEntryCount, pCreateInfos[createInfoIndex].stage.pSpecializationInfo->pMapEntries, false, true);

                skipCall |= validate_array(report_data, "vkCreateComputePipelines", "pCreateInfos[i].stage.pSpecializationInfo->dataSize", "pCreateInfos[i].stage.pSpecializationInfo->pData", pCreateInfos[createInfoIndex].stage.pSpecializationInfo->dataSize, pCreateInfos[createInfoIndex].stage.pSpecializationInfo->pData, false, true);
            }

            skipCall |= validate_required_handle(report_data, "vkCreateComputePipelines", "pCreateInfos[i].layout", pCreateInfos[createInfoIndex].layout);
        }
    }
    ...
    VkPipelineLayout*                           pPipelineLayout)
{
    ...
    if (pCreateInfo != NULL)
    {
        ...
        if (pCreateInfo->pPushConstantRanges != NULL)
        {
            ...
            {
                skipCall |= validate_flags(report_data, "vkCreatePipelineLayout", "pCreateInfo->pPushConstantRanges[i].stageFlags", "VkShaderStageFlagBits", AllVkShaderStageFlagBits, pCreateInfo->pPushConstantRanges[pushConstantRangeIndex].stageFlags, true);
            }
        }
    }
    ...
    VkDescriptorSetLayout*                      pSetLayout)
{
    ...
    if (pCreateInfo != NULL)
    {
        ...
        if (pCreateInfo->pBindings != NULL)
        {
            ...
            {
                skipCall |= validate_ranged_enum(report_data, "vkCreateDescriptorSetLayout", "pCreateInfo->pBindings[i].descriptorType", "VkDescriptorType", VK_DESCRIPTOR_TYPE_BEGIN_RANGE, VK_DESCRIPTOR_TYPE_END_RANGE, pCreateInfo->pBindings[bindingIndex].descriptorType);
            }
        }
    }
    ...
    VkDescriptorPool*                           pDescriptorPool)
{
    ...
    if (pCreateInfo != NULL)
    {
        ...
        if (pCreateInfo->pPoolSizes != NULL)
        {
            ...
            {
                skipCall |= validate_ranged_enum(report_data, "vkCreateDescriptorPool", "pCreateInfo->pPoolSizes[i].type", "VkDescriptorType", VK_DESCRIPTOR_TYPE_BEGIN_RANGE, VK_DESCRIPTOR_TYPE_END_RANGE, pCreateInfo->pPoolSizes[poolSizeIndex].type);
            }
        }
    }
    ...
    const VkCopyDescriptorSet*                  pDescriptorCopies)
{
    ...
    if (pDescriptorWrites != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkUpdateDescriptorSets", "pDescriptorWrites[i].pNext", NULL, pDescriptorWrites[descriptorWriteIndex].pNext, 0, NULL);

            skipCall |= validate_required_handle(report_data, "vkUpdateDescriptorSets", "pDescriptorWrites[i].dstSet", pDescriptorWrites[descriptorWriteIndex].dstSet);

            skipCall |= validate_ranged_enum(report_data, "vkUpdateDescriptorSets", "pDescriptorWrites[i].descriptorType", "VkDescriptorType", VK_DESCRIPTOR_TYPE_BEGIN_RANGE, VK_DESCRIPTOR_TYPE_END_RANGE, pDescriptorWrites[descriptorWriteIndex].descriptorType);

            if (pDescriptorWrites[descriptorWriteIndex].pBufferInfo != NULL)
            {
                for (uint32_t descriptorIndex = 0; descriptorIndex < pDescriptorWrites[descriptorWriteIndex].descriptorCount; ++descriptorIndex)
                {
                    skipCall |= validate_required_handle(report_data, "vkUpdateDescriptorSets", "pDescriptorWrites[i].pBufferInfo[i].buffer", pDescriptorWrites[descriptorWriteIndex].pBufferInfo[descriptorIndex].buffer);
                }
            }
        }
    }
    ...
    if (pDescriptorCopies != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkUpdateDescriptorSets", "pDescriptorCopies[i].pNext", NULL, pDescriptorCopies[descriptorCopyIndex].pNext, 0, NULL);

            skipCall |= validate_required_handle(report_data, "vkUpdateDescriptorSets", "pDescriptorCopies[i].srcSet", pDescriptorCopies[descriptorCopyIndex].srcSet);

            skipCall |= validate_required_handle(report_data, "vkUpdateDescriptorSets", "pDescriptorCopies[i].dstSet", pDescriptorCopies[descriptorCopyIndex].dstSet);
        }
    }
    ...
    VkFramebuffer*                              pFramebuffer)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateFramebuffer", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkRenderPass*                               pRenderPass)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateRenderPass", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
        ...
        if (pCreateInfo->pAttachments != NULL)
        {
            ...
            {
                skipCall |= validate_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].flags", "VkAttachmentDescriptionFlagBits", AllVkAttachmentDescriptionFlagBits, pCreateInfo->pAttachments[attachmentIndex].flags, false);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].format", "VkFormat", VK_FORMAT_BEGIN_RANGE, VK_FORMAT_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].format);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].loadOp", "VkAttachmentLoadOp", VK_ATTACHMENT_LOAD_OP_BEGIN_RANGE, VK_ATTACHMENT_LOAD_OP_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].loadOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].storeOp", "VkAttachmentStoreOp", VK_ATTACHMENT_STORE_OP_BEGIN_RANGE, VK_ATTACHMENT_STORE_OP_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].storeOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].stencilLoadOp", "VkAttachmentLoadOp", VK_ATTACHMENT_LOAD_OP_BEGIN_RANGE, VK_ATTACHMENT_LOAD_OP_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].stencilLoadOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].stencilStoreOp", "VkAttachmentStoreOp", VK_ATTACHMENT_STORE_OP_BEGIN_RANGE, VK_ATTACHMENT_STORE_OP_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].stencilStoreOp);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].initialLayout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].initialLayout);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pAttachments[i].finalLayout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pCreateInfo->pAttachments[attachmentIndex].finalLayout);
            }
        }
        ...
        if (pCreateInfo->pSubpasses != NULL)
        {
            ...
            {
                skipCall |= validate_reserved_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].flags", pCreateInfo->pSubpasses[subpassIndex].flags);

                skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].pipelineBindPoint", "VkPipelineBindPoint", VK_PIPELINE_BIND_POINT_BEGIN_RANGE, VK_PIPELINE_BIND_POINT_END_RANGE, pCreateInfo->pSubpasses[subpassIndex].pipelineBindPoint);

                skipCall |= validate_array(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].inputAttachmentCount", "pCreateInfo->pSubpasses[i].pInputAttachments", pCreateInfo->pSubpasses[subpassIndex].inputAttachmentCount, pCreateInfo->pSubpasses[subpassIndex].pInputAttachments, false, true);
                ...
                if (pCreateInfo->pSubpasses[subpassIndex].pInputAttachments != NULL)
                {
                    ...
                    {
                        skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].pInputAttachments[i].layout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pCreateInfo->pSubpasses[subpassIndex].pInputAttachments[inputAttachmentIndex].layout);
                    }
                }

                skipCall |= validate_array(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].colorAttachmentCount", "pCreateInfo->pSubpasses[i].pColorAttachments", pCreateInfo->pSubpasses[subpassIndex].colorAttachmentCount, pCreateInfo->pSubpasses[subpassIndex].pColorAttachments, false, true);
                ...
                if (pCreateInfo->pSubpasses[subpassIndex].pColorAttachments != NULL)
                {
                    ...
                    {
                        skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].pColorAttachments[i].layout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pCreateInfo->pSubpasses[subpassIndex].pColorAttachments[colorAttachmentIndex].layout);
                    }
                }
                ...
                if (pCreateInfo->pSubpasses[subpassIndex].pResolveAttachments != NULL)
                {
                    for (uint32_t colorAttachmentIndex = 0; colorAttachmentIndex < pCreateInfo->pSubpasses[subpassIndex].colorAttachmentCount; ++colorAttachmentIndex)
                    {
                        skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].pResolveAttachments[i].layout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pCreateInfo->pSubpasses[subpassIndex].pResolveAttachments[colorAttachmentIndex].layout);
                    }
                }

                if (pCreateInfo->pSubpasses[subpassIndex].pDepthStencilAttachment != NULL)
                {
                    skipCall |= validate_ranged_enum(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].pDepthStencilAttachment->layout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pCreateInfo->pSubpasses[subpassIndex].pDepthStencilAttachment->layout);
                }

                skipCall |= validate_array(report_data, "vkCreateRenderPass", "pCreateInfo->pSubpasses[i].preserveAttachmentCount", "pCreateInfo->pSubpasses[i].pPreserveAttachments", pCreateInfo->pSubpasses[subpassIndex].preserveAttachmentCount, pCreateInfo->pSubpasses[subpassIndex].pPreserveAttachments, false, true);
            }
        }
        ...
        if (pCreateInfo->pDependencies != NULL)
        {
            ...
            {
                skipCall |= validate_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pDependencies[i].srcStageMask", "VkPipelineStageFlagBits", AllVkPipelineStageFlagBits, pCreateInfo->pDependencies[dependencyIndex].srcStageMask, true);

                skipCall |= validate_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pDependencies[i].dstStageMask", "VkPipelineStageFlagBits", AllVkPipelineStageFlagBits, pCreateInfo->pDependencies[dependencyIndex].dstStageMask, true);

                skipCall |= validate_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pDependencies[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pCreateInfo->pDependencies[dependencyIndex].srcAccessMask, false);

                skipCall |= validate_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pDependencies[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pCreateInfo->pDependencies[dependencyIndex].dstAccessMask, false);

                skipCall |= validate_flags(report_data, "vkCreateRenderPass", "pCreateInfo->pDependencies[i].dependencyFlags", "VkDependencyFlagBits", AllVkDependencyFlagBits, pCreateInfo->pDependencies[dependencyIndex].dependencyFlags, false);
            }
        }
    }
    ...
    VkCommandPool*                              pCommandPool)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateCommandPool", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkCommandBuffer*                            pCommandBuffers)
{
    ...
    if (pAllocateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkAllocateCommandBuffers", "pAllocateInfo->pNext", NULL, pAllocateInfo->pNext, 0, NULL);
    ...
    const VkCommandBufferBeginInfo*             pBeginInfo)
{
    ...
    if (pBeginInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkBeginCommandBuffer", "pBeginInfo->pNext", NULL, pBeginInfo->pNext, 0, NULL);
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkBeginCommandBuffer", "pBeginInfo->pInheritanceInfo->pNext", NULL, pBeginInfo->pInheritanceInfo->pNext, 0, NULL);

            skipCall |= validate_bool32(report_data, "vkBeginCommandBuffer", "pBeginInfo->pInheritanceInfo->occlusionQueryEnable", pBeginInfo->pInheritanceInfo->occlusionQueryEnable);
        }
    }
    ...
    const VkImageCopy*                          pRegions)
{
    ...
    if (pRegions != NULL)
    {
        ...
        {
            skipCall |= validate_flags(report_data, "vkCmdCopyImage", "pRegions[i].srcSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].srcSubresource.aspectMask, true);

            skipCall |= validate_flags(report_data, "vkCmdCopyImage", "pRegions[i].dstSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].dstSubresource.aspectMask, true);
        }
    }
    ...
    VkFilter                                    filter)
{
    ...
    if (pRegions != NULL)
    {
        for (uint32_t regionIndex = 0; regionIndex < regionCount; ++regionIndex)
        {
            skipCall |= validate_flags(report_data, "vkCmdBlitImage", "pRegions[i].srcSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].srcSubresource.aspectMask, true);

            skipCall |= validate_flags(report_data, "vkCmdBlitImage", "pRegions[i].dstSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].dstSubresource.aspectMask, true);
        }
    }
    ...
    const VkBufferImageCopy*                    pRegions)
{
    ...
    if (pRegions != NULL)
    {
        for (uint32_t regionIndex = 0; regionIndex < regionCount; ++regionIndex)
        {
            skipCall |= validate_flags(report_data, "vkCmdCopyBufferToImage", "pRegions[i].imageSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].imageSubresource.aspectMask, true);
        }
    }
    ...
    const VkBufferImageCopy*                    pRegions)
{
    ...
    if (pRegions != NULL)
    {
        for (uint32_t regionIndex = 0; regionIndex < regionCount; ++regionIndex)
        {
            skipCall |= validate_flags(report_data, "vkCmdCopyImageToBuffer", "pRegions[i].imageSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].imageSubresource.aspectMask, true);
        }
    }
    ...
    const VkImageSubresourceRange*              pRanges)
{
    ...
    if (pRanges != NULL)
    {
        ...
        {
            skipCall |= validate_flags(report_data, "vkCmdClearColorImage", "pRanges[i].aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRanges[rangeIndex].aspectMask, true);
        }
    }
    ...
    const VkImageSubresourceRange*              pRanges)
{
    ...
    if (pRanges != NULL)
    {
        for (uint32_t rangeIndex = 0; rangeIndex < rangeCount; ++rangeIndex)
        {
            skipCall |= validate_flags(report_data, "vkCmdClearDepthStencilImage", "pRanges[i].aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRanges[rangeIndex].aspectMask, true);
        }
    }
    ...
    const VkClearRect*                          pRects)
{
    ...
    if (pAttachments != NULL)
    {
        ...
        {
            skipCall |= validate_flags(report_data, "vkCmdClearAttachments", "pAttachments[i].aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pAttachments[attachmentIndex].aspectMask, true);
        }
    }
    ...
    const VkImageResolve*                       pRegions)
{
    ...
    if (pRegions != NULL)
    {
        for (uint32_t regionIndex = 0; regionIndex < regionCount; ++regionIndex)
        {
            skipCall |= validate_flags(report_data, "vkCmdResolveImage", "pRegions[i].srcSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].srcSubresource.aspectMask, true);

            skipCall |= validate_flags(report_data, "vkCmdResolveImage", "pRegions[i].dstSubresource.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pRegions[regionIndex].dstSubresource.aspectMask, true);
        }
    }
    ...
    const VkImageMemoryBarrier*                 pImageMemoryBarriers)
{
    ...
    if (pMemoryBarriers != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkCmdWaitEvents", "pMemoryBarriers[i].pNext", NULL, pMemoryBarriers[memoryBarrierIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pMemoryBarriers[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pMemoryBarriers[memoryBarrierIndex].srcAccessMask, false);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pMemoryBarriers[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pMemoryBarriers[memoryBarrierIndex].dstAccessMask, false);
        }
    }
    ...
    if (pBufferMemoryBarriers != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkCmdWaitEvents", "pBufferMemoryBarriers[i].pNext", NULL, pBufferMemoryBarriers[bufferMemoryBarrierIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pBufferMemoryBarriers[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pBufferMemoryBarriers[bufferMemoryBarrierIndex].srcAccessMask, false);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pBufferMemoryBarriers[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pBufferMemoryBarriers[bufferMemoryBarrierIndex].dstAccessMask, false);

            skipCall |= validate_required_handle(report_data, "vkCmdWaitEvents", "pBufferMemoryBarriers[i].buffer", pBufferMemoryBarriers[bufferMemoryBarrierIndex].buffer);
        }
    }
    ...
    if (pImageMemoryBarriers != NULL)
    {
        ...
        {
            skipCall |= validate_struct_pnext(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].pNext", NULL, pImageMemoryBarriers[imageMemoryBarrierIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pImageMemoryBarriers[imageMemoryBarrierIndex].srcAccessMask, false);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pImageMemoryBarriers[imageMemoryBarrierIndex].dstAccessMask, false);

            skipCall |= validate_ranged_enum(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].oldLayout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pImageMemoryBarriers[imageMemoryBarrierIndex].oldLayout);

            skipCall |= validate_ranged_enum(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].newLayout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pImageMemoryBarriers[imageMemoryBarrierIndex].newLayout);

            skipCall |= validate_required_handle(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].image", pImageMemoryBarriers[imageMemoryBarrierIndex].image);

            skipCall |= validate_flags(report_data, "vkCmdWaitEvents", "pImageMemoryBarriers[i].subresourceRange.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pImageMemoryBarriers[imageMemoryBarrierIndex].subresourceRange.aspectMask, true);
        }
    }
    ...
    const VkImageMemoryBarrier*                 pImageMemoryBarriers)
{
    ...
    if (pMemoryBarriers != NULL)
    {
        for (uint32_t memoryBarrierIndex = 0; memoryBarrierIndex < memoryBarrierCount; ++memoryBarrierIndex)
        {
            skipCall |= validate_struct_pnext(report_data, "vkCmdPipelineBarrier", "pMemoryBarriers[i].pNext", NULL, pMemoryBarriers[memoryBarrierIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pMemoryBarriers[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pMemoryBarriers[memoryBarrierIndex].srcAccessMask, false);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pMemoryBarriers[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pMemoryBarriers[memoryBarrierIndex].dstAccessMask, false);
        }
    }
    ...
    if (pBufferMemoryBarriers != NULL)
    {
        for (uint32_t bufferMemoryBarrierIndex = 0; bufferMemoryBarrierIndex < bufferMemoryBarrierCount; ++bufferMemoryBarrierIndex)
        {
            skipCall |= validate_struct_pnext(report_data, "vkCmdPipelineBarrier", "pBufferMemoryBarriers[i].pNext", NULL, pBufferMemoryBarriers[bufferMemoryBarrierIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pBufferMemoryBarriers[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pBufferMemoryBarriers[bufferMemoryBarrierIndex].srcAccessMask, false);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pBufferMemoryBarriers[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pBufferMemoryBarriers[bufferMemoryBarrierIndex].dstAccessMask, false);

            skipCall |= validate_required_handle(report_data, "vkCmdPipelineBarrier", "pBufferMemoryBarriers[i].buffer", pBufferMemoryBarriers[bufferMemoryBarrierIndex].buffer);
        }
    }
    ...
    if (pImageMemoryBarriers != NULL)
    {
        for (uint32_t imageMemoryBarrierIndex = 0; imageMemoryBarrierIndex < imageMemoryBarrierCount; ++imageMemoryBarrierIndex)
        {
            skipCall |= validate_struct_pnext(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].pNext", NULL, pImageMemoryBarriers[imageMemoryBarrierIndex].pNext, 0, NULL);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].srcAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pImageMemoryBarriers[imageMemoryBarrierIndex].srcAccessMask, false);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].dstAccessMask", "VkAccessFlagBits", AllVkAccessFlagBits, pImageMemoryBarriers[imageMemoryBarrierIndex].dstAccessMask, false);

            skipCall |= validate_ranged_enum(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].oldLayout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pImageMemoryBarriers[imageMemoryBarrierIndex].oldLayout);

            skipCall |= validate_ranged_enum(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].newLayout", "VkImageLayout", VK_IMAGE_LAYOUT_BEGIN_RANGE, VK_IMAGE_LAYOUT_END_RANGE, pImageMemoryBarriers[imageMemoryBarrierIndex].newLayout);

            skipCall |= validate_required_handle(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].image", pImageMemoryBarriers[imageMemoryBarrierIndex].image);

            skipCall |= validate_flags(report_data, "vkCmdPipelineBarrier", "pImageMemoryBarriers[i].subresourceRange.aspectMask", "VkImageAspectFlagBits", AllVkImageAspectFlagBits, pImageMemoryBarriers[imageMemoryBarrierIndex].subresourceRange.aspectMask, true);
        }
    }
    ...
    VkSubpassContents                           contents)
{
    ...
    {
        skipCall |= validate_struct_pnext(report_data, "vkCmdBeginRenderPass", "pRenderPassBegin->pNext", NULL, pRenderPassBegin->pNext, 0, NULL);
    ...
    VkSwapchainKHR*                             pSwapchain)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateSwapchainKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    const VkPresentInfoKHR*                     pPresentInfo)
{
    ...
    {
        skipCall |= validate_struct_pnext(report_data, "vkQueuePresentKHR", "pPresentInfo->pNext", NULL, pPresentInfo->pNext, 0, NULL);
    ...
    VkDisplayModeKHR*                           pMode)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateDisplayModeKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateDisplayPlaneSurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateXlibSurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateXcbSurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateWaylandSurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateMirSurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateAndroidSurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
    ...
    VkSurfaceKHR*                               pSurface)
{
    ...
    if (pCreateInfo != NULL)
    {
        skipCall |= validate_struct_pnext(report_data, "vkCreateWin32SurfaceKHR", "pCreateInfo->pNext", NULL, pCreateInfo->pNext, 0, NULL);
